...
from app.schemas.workout import WorkoutCreate, WorkoutResponse, WorkoutUpdate, WorkoutWithExercises, WorkoutExerciseCreate, WorkoutExerciseUpdate
...
def create_workout(workout: WorkoutCreate, repo: WorkoutsRepository = Depends(get_workouts_repository)):
    return repo.create_workout(workout)

@router.get("/", response_model=List[WorkoutResponse])
def list_workouts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    # Get workouts with exercise instances, their definitions, and sets
    ...
    for workout in workouts:
        safe_name = workout.name if isinstance(workout.name, str) and len(workout.name) > 0 else f"Workout #{workout.id}"
        workout_data = {
            "id": workout.id,
            "name": safe_name,
        ...
        for instance in workout.exercise_instances:
            ...
            if instance.exercise_definition:
                instance_data["exercise_definition"] = {
                    ...
                    "target_muscles": getattr(instance.exercise_definition, 'target_muscles', None),
                    "synergist_muscles": getattr(instance.exercise_definition, 'synergist_muscles', None),
                }
...
def update_workout(workout_id: int, workout: WorkoutUpdate, repo: WorkoutsRepository = Depends(get_workouts_repository)):
    try:
