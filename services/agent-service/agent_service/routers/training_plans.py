...
async def generate_plan(user_data: UserDataInput, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    """
    ...
    await save_plan_to_plans_service(plan, user_id)
    await notify_rpe_plan_created(plan, user_id)
    return plan


@router.post('/generate-plan-with-rationale/', response_model=TrainingPlanWithRationale)
async def generate_plan_with_rationale(user_data: UserDataInput, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
async def generate_plan_with_summary(user_data: UserDataInput, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def save_generated_plan(db: Session, plan: TrainingPlan, user_id: str):
    """
