...
CONVERSATION_STATE_PROMPTS: Dict[str, Dict[str, List[str] | str]] = {
    ...
    "collect_constraints": {
        "system": dedent(
            """
            Ты фитнес-ассистент. Выясни ограничения пользователя по времени и возможностям. Требуемые данные:
...
EXTRACT_USER_DATA_SYSTEM_PROMPT: str = dedent(
    ...
    3. Если пользователь указал длительность (в неделях/месяцах), переведи в целые недели и запиши в
       plan_duration_weeks.
    4. Для available_equipment используй английские названия. Если говорится «любое оборудование» или
       ограничений нет, можно вернуть полный набор: ["barbell", "dumbbells", "machine", "cable",
       "bodyweight"].
...
STATE_COMPLETION_PROMPT_TEMPLATE: str = dedent(
    ...
    Если вердикт INCOMPLETE, добавь еще одну строку 'NEXT_QUESTION: <один уточняющий вопрос на русском языке>' без дополнительных пояснений.
    """
).strip()
