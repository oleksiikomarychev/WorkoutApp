...
from exercises_service.dependencies import get_db, get_set_service

router = APIRouter(prefix="/instances")

@router.get("/{instance_id}", response_model=schemas.ExerciseInstanceResponse)
async def get_exercise_instance(instance_id: int, db: AsyncSession = Depends(get_db)):
    db_instance = await ExerciseRepository.get_exercise_instance(db, instance_id)
    if not db_instance:
...
async def create_exercise_instance(workout_id: int, instance_data: schemas.ExerciseInstanceCreate, db: AsyncSession = Depends(get_db), set_service: SetService = Depends(get_set_service)):
    service = ExerciseInstanceService(db, set_service)
    try:
...
async def update_exercise_instance(instance_id: int, instance_update: schemas.ExerciseInstanceBase, db: AsyncSession = Depends(get_db)):
    service = ExerciseInstanceService(db, SetService())
...
async def update_exercise_set(instance_id: int, set_id: int, payload: schemas.ExerciseSetUpdate, db: AsyncSession = Depends(get_db), set_service: SetService = Depends(get_set_service)):
    service = ExerciseInstanceService(db, set_service)
...
async def delete_exercise_set(instance_id: int, set_id: int, db: AsyncSession = Depends(get_db)):
    db_instance = await ExerciseRepository.get_exercise_instance(db, instance_id)
    if not db_instance:
        raise HTTPException(status_code=404, detail="Exercise instance not found")
...
async def delete_exercise_instance(instance_id: int, db: AsyncSession = Depends(get_db)):
    await ExerciseRepository.delete_exercise_instance(db, instance_id)

@router.post("/batch", response_model=list[schemas.ExerciseInstanceResponse])
async def create_exercise_instances_batch(instances_data: list[schemas.ExerciseInstanceCreate], db: AsyncSession = Depends(get_db)):
    instances_list = []
    ...
    return await ExerciseRepository.create_exercise_instances_batch(db, instances_list)

@router.post("/migrate-set-ids", status_code=status.HTTP_200_OK)
async def migrate_set_ids(db: AsyncSession = Depends(get_db)):
...
async def get_instances_by_workout(workout_id: int, db: AsyncSession = Depends(get_db)):
    db_instances = await ExerciseRepository.get_instances_by_workout(db, workout_id)
    if not db_instances:
        return []
    return [{
        "id": i.id,
