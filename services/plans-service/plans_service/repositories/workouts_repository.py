...
from ..schemas.workout import WorkoutResponse
...
class WorkoutsRepository:
    ...
    def update_workout(self, workout_id: int, workout: WorkoutResponse) -> Workout:
        ...
        if (
            hasattr(workout, "started_at")
            and getattr(workout, "started_at") is not None
        ):
            db_workout.started_at = workout.started_at
        if (
            hasattr(workout, "duration_seconds")
            and getattr(workout, "duration_seconds") is not None
        ):
            db_workout.duration_seconds = workout.duration_seconds
        if (
            hasattr(workout, "rpe_session")
            and getattr(workout, "rpe_session") is not None
        ):
        ...
        if (
            hasattr(workout, "readiness_score")
            and getattr(workout, "readiness_score") is not None
        ):
            db_workout.readiness_score = workout.readiness_score
        if (
            hasattr(workout, "applied_plan_id")
            and getattr(workout, "applied_plan_id") is not None
        ):
            db_workout.applied_plan_id = workout.applied_plan_id
        if (
            hasattr(workout, "plan_order_index")
            and getattr(workout, "plan_order_index") is not None
        ):
            db_workout.plan_order_index = workout.plan_order_index
        if (
            hasattr(workout, "scheduled_for")
            and getattr(workout, "scheduled_for") is not None
        ):
            db_workout.scheduled_for = workout.scheduled_for
        if (
            hasattr(workout, "completed_at")
            and getattr(workout, "completed_at") is not None
        ):
            db_workout.completed_at = workout.completed_at

        # Update exercise instances if provided
        if hasattr(workout, "exercise_instances") and isinstance(
            workout.exercise_instances, list
        ):
            ...
            for instance_data in workout.exercise_instances:
                ...
                ex_list_id = (
                    getattr(instance_data, "exercise_list_id", None)
                    if not isinstance(instance_data, dict)
                    else instance_data.get("exercise_list_id")
                )
                user_max_id = (
                    getattr(instance_data, "user_max_id", None)
                    if not isinstance(instance_data, dict)
                    else instance_data.get("user_max_id")
                )
                sets_raw = (
                    getattr(instance_data, "sets", None)
                    if not isinstance(instance_data, dict)
                    else instance_data.get("sets")
                )
                notes = (
                    getattr(instance_data, "notes", None)
                    if not isinstance(instance_data, dict)
                    else instance_data.get("notes")
                )
                order = (
                    getattr(instance_data, "order", None)
                    if not isinstance(instance_data, dict)
                    else instance_data.get("order")
                )
                ...
                for s in sets_raw:
                    if hasattr(s, "model_dump"):
                        normalized_sets.append(
                            _apply_inbound_mapping_to_set(s.model_dump())
                        )
                    else:
