...
class UserMaxRepository:
    ...
    def get_user_max_by_id(self, user_max_id: int) -> Optional[UserMax]:
        # Relationship to ExerciseList was removed for microservice isolation; load plain entity
        return self.db.query(UserMax).filter(UserMax.id == user_max_id).first()

    def get_user_maxes_by_user(
        self, user_id: int, skip: int = 0, limit: int = 100
    ) -> List[UserMax]:
        # No eager loads; method retained for compatibility though user_id isn't part of this model here
        return (
            self.db.query(UserMax)
            .filter(UserMax.user_id == user_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def delete_user_max(self, user_max_id: int, user_id: Optional[int] = None) -> bool:
        query = self.db.query(UserMax).filter(UserMax.id == user_max_id)
    ...
    def get_or_create_user_max(
        self, user_id: int, exercise_id: int, defaults: Optional[Dict[str, Any]] = None
    ) -> UserMax:
        if defaults is None:
            defaults = {}
