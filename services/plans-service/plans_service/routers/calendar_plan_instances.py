...
from ..services.calendar_plan_instance_service import CalendarPlanInstanceService
...
) -> List[CalendarPlanInstanceResponse]:
    service = CalendarPlanInstanceService(db)
    return service.list_instances(user_id)


@router.post(
    "", response_model=CalendarPlanInstanceResponse, status_code=status.HTTP_201_CREATED
)
async def create_instance(
    payload: CalendarPlanInstanceCreate,
...
async def get_instance(
    instance_id: int, db: Session = Depends(get_db), user_id: str = Depends(get_user_id)
) -> CalendarPlanInstanceResponse:
    service = CalendarPlanInstanceService(db)
    inst = service.get_instance(instance_id, user_id)
    if not inst:
...
async def update_instance(
    instance_id: int,
    payload: CalendarPlanInstanceUpdate,
    db: Session = Depends(get_db),
    user_id: str = Depends(get_user_id),
) -> CalendarPlanInstanceResponse:
    service = CalendarPlanInstanceService(db)
    try:
        return service.update(instance_id, payload, user_id)
...
async def delete_instance(
    ...
    service = CalendarPlanInstanceService(db)
    service.delete_instance(instance_id, user_id)


@router.post(
    "/from-plan/{plan_id}",
    response_model=CalendarPlanInstanceResponse,
...
async def create_from_plan(
    ...
    service = CalendarPlanInstanceService(db)
    try:
        return service.create_from_plan(plan_id, user_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.post("/{instance_id}/apply", response_model=AppliedCalendarPlanResponse)
async def apply_from_instance(
    instance_id: int,
    ...
    try:
        return service.apply_plan_from_instance(
            instance_id, user_id, payload.user_max_ids, payload.compute
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
