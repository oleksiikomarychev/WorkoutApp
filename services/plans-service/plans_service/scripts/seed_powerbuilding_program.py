...
def get_upper_body_hypertrophy_day(db: Session) -> List[Dict[str, Any]]:
    ...
    exercises = [
        _create_exercise(_get_exercise_id(db, "Жим лёжа"), [
            {"intensity": 70, "volume": 10, "effort": 7},
            {"intensity": 75, "volume": 8, "effort": 8},
            {"intensity": 75, "volume": 8, "effort": 8},
            {"intensity": 75, "volume": 8, "effort": 9},
        ]),
        _create_exercise(_get_exercise_id(db, "Тяга штанги в наклоне"), [
            {"intensity": 70, "volume": 10, "effort": 8},
            {"intensity": 70, "volume": 10, "effort": 8},
            {"intensity": 70, "volume": 10, "effort": 9},
        ]),
        _create_exercise(_get_exercise_id(db, "Жим стоя"), [
            {"intensity": 65, "volume": 12, "effort": 8},
            {"intensity": 65, "volume": 12, "effort": 9},
            {"intensity": 65, "volume": 12, "effort": 9},
        ]),
        _create_exercise(_get_exercise_id(db, "Подтягивания"), [
            {"intensity": 0, "volume": 10, "effort": 9}, # Bodyweight or with assistance
            {"intensity": 0, "volume": 10, "effort": 9},
            {"intensity": 0, "volume": 10, "effort": 10},
        ]),
        _create_exercise(_get_exercise_id(db, "Разгибание на блоке"), [
...
def get_lower_body_hypertrophy_day(db: Session) -> List[Dict[str, Any]]:
    ...
    exercises = [
        _create_exercise(_get_exercise_id(db, "Приседания со штангой"), [
            {"intensity": 70, "volume": 10, "effort": 7},
            {"intensity": 75, "volume": 8, "effort": 8},
            {"intensity": 75, "volume": 8, "effort": 8},
            {"intensity": 75, "volume": 8, "effort": 9},
        ]),
        _create_exercise(_get_exercise_id(db, "Румынская тяга"), [
            {"intensity": 65, "volume": 12, "effort": 8},
            {"intensity": 65, "volume": 12, "effort": 8},
            {"intensity": 65, "volume": 12, "effort": 9},
        ]),
        _create_exercise(_get_exercise_id(db, "Жим ногами"), [
            {"intensity": 70, "volume": 15, "effort": 9},
            {"intensity": 70, "volume": 15, "effort": 9},
            {"intensity": 70, "volume": 15, "effort": 10},
        ]),
        _create_exercise(_get_exercise_id(db, "Сгибание ног"), [
            {"intensity": 60, "volume": 15, "effort": 9},
            {"intensity": 60, "volume": 15, "effort": 10},
        ]),
    ]
...
def get_lower_body_strength_day(db: Session) -> List[Dict[str, Any]]:
    ...
    exercises = [
        ...
        _create_exercise(_get_exercise_id(db, "Становая тяга"), [
            {"intensity": 80, "volume": 5, "effort": 8},
            {"intensity": 85, "volume": 3, "effort": 9},
        ]),
    ]
...
def get_deload_day(db: Session) -> List[Dict[str, Any]]:
    ...
    exercises = [
        ...
        _create_exercise(_get_exercise_id(db, "Тяга блока"), [
...
def seed_program(plan_name: str, drop_existing: bool = True) -> None:
    ...
    try:
        ...
        mesocycles_data = [
            ...
            {"name": "Силовая интенсификация", "microcycles_count": 6, "days_per_micro": 6},
...
def main() -> None:
    ...
    parser.add_argument("--name", type=str, default="Программа 'ПРОГРЕСС ЖИМ/НОГИ'", help="Name for the training plan.")
    parser.add_argument("--no-drop", action="store_true", help="Do not delete an existing plan with the same name.")
