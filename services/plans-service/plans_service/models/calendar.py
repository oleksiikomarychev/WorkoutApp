...
class AppliedCalendarPlanUserMax(Base):
    ...
    applied_calendar_plan_id = Column(
        Integer,
        ForeignKey("applied_calendar_plans.id", ondelete="CASCADE"),
        primary_key=True,
    )
    user_max_id = Column(
        Integer, ForeignKey("user_maxes.id", ondelete="CASCADE"), primary_key=True
    )
...
class AppliedCalendarPlan(Base):
    ...
    user_maxes = relationship(
        "UserMax",
        back_populates="applied_plans",
        secondary="applied_calendar_plan_user_maxes",
    )
    # Link generated workouts
    workouts = relationship(
        "Workout", back_populates="applied_plan", cascade="all, delete-orphan"
    )
...
class Microcycle(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    mesocycle_id = Column(
        Integer, ForeignKey("mesocycles.id", ondelete="CASCADE"), nullable=False
    )
    name = Column(String(255), nullable=False)
    notes = Column(String(100), nullable=True)
    order_index = Column(Integer, nullable=False, default=0)
...
class CalendarPlanInstance(Base):
    ...
    source_plan_id = Column(
        Integer, ForeignKey("calendar_plans.id", ondelete="SET NULL"), nullable=True
    )
    name = Column(String(255), nullable=False)
    schedule: Dict[str, List[Dict[str, Any]]] = Column(JSON, nullable=False)
    duration_weeks = Column(Integer, nullable=False)
