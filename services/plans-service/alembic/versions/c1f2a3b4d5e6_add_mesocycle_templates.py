...
def upgrade():
    ...
    existing_meso_idx = {ix['name'] for ix in insp.get_indexes('mesocycle_templates')} if 'mesocycle_templates' in insp.get_table_names() else set()
    ...
    if 'microcycle_templates' not in insp.get_table_names():
        op.create_table(
            'microcycle_templates',
            sa.Column('id', sa.Integer(), primary_key=True, nullable=False),
            sa.Column('mesocycle_template_id', sa.Integer(), sa.ForeignKey('mesocycle_templates.id', ondelete='CASCADE'), nullable=False),
            sa.Column('name', sa.String(length=255), nullable=False),
            sa.Column('notes', sa.String(length=255), nullable=True),
    ...
    existing_micro_idx = {ix['name'] for ix in insp.get_indexes('microcycle_templates')} if 'microcycle_templates' in insp.get_table_names() else set()
    ...
    if 'ix_microcycle_templates_mesocycle_template_id' not in existing_micro_idx:
        try:
            op.create_index('ix_microcycle_templates_mesocycle_template_id', 'microcycle_templates', ['mesocycle_template_id'], unique=False)
        except Exception:
            pass
