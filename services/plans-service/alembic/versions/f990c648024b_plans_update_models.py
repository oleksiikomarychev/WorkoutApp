...
def upgrade():
    ...
    op.add_column(
        ...
        sa.Column("actual_sessions_completed", sa.Integer(), nullable=True),
    )
    ...
    op.add_column(
        "calendar_plans",
        sa.Column("intended_experience_level", sa.String(length=32), nullable=True),
    )
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "id",
            sa.INTEGER(),
            server_default=sa.text("nextval('mesocycle_templates_id_seq'::regclass)"),
            autoincrement=True,
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["mesocycle_templates.id"],
            name=op.f("microcycle_templates_mesocycle_template_id_fkey"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("microcycle_templates_pkey")),
    )
    op.create_index(
        op.f("ix_microcycle_templates_mesocycle_template_id"),
        "microcycle_templates",
        ["mesocycle_template_id"],
        unique=False,
    )
