...
def upgrade():
    ...
    if "exercise_definition_id" not in existing_columns:
        op.add_column("plan_exercises", sa.Column("exercise_definition_id", sa.Integer(), nullable=True))

    if "exercise_name" not in existing_columns:
        op.add_column("plan_exercises", sa.Column("exercise_name", sa.String(length=255), nullable=True))
    ...
    if "created_at" not in existing_columns:
        op.add_column("plan_exercises", sa.Column("created_at", sa.DateTime(), server_default=sa.func.now()))
