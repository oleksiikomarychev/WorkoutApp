...
from ..services import fetch_current_user, reactivate_or_create_link
...
def create_invitation(payload: InvitationCreate, db: Session = Depends(get_db), uid: str = Depends(get_current_uid)):
    coach = fetch_current_user(uid)
    # Basic guardrails: avoid inviting self
    if payload.email_or_user_id in (coach["id"], coach.get("firebase_uid"), coach.get("email")):
